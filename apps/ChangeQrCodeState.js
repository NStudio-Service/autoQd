...
function setFormData(id, value) {
	...
	let gzipTxt = http.postJson("http://ijg.xujc.com/api/formEngine/formInstance/" + id, {
		...
		headers: {
			...
			"Referer": "http://ijg.xujc.com/app/235",
...
function sendAndReceive(formId, state) {
	let finalRes = JSON.parse(setFormData(formId, state)), nowHealth;
	finalRes.data.formData.forEach(function (item) {
		if (item.title === "手动修改健康码") {
			nowHealth = item.value.stringValue
		}
	});
	if (!nowHealth) {
		finalRes.data.formData.forEach(function (item) {
			if (item.title === "连续14天的身体健康状况") {
				nowHealth = item.value.stringValue
			}
		});
	}
...
events.broadcast.on("httpRes", function (res) {
	if (res.state) {
		ui.run(function () {
			...
			ui.healthTxt.setText(res.nowHealth);
		});
	} else {
		ui.run(function () {
			ui.loading1.setVisibility(8);
...
//获得表单数据
	...
	ui.run(function () {
		let warning = ui.inflate(
			<vertical paddingTop="250dip" paddingLeft="30dip" paddingRight="30dip">
				<vertical background="@drawable/popup_background">
					<text padding="15" text="你将以“学工部”的身份为健康码调制颜色，请做出选择："/>
					<linear gravity="center">
						<button style="Widget.AppCompat.Button.Borderless.Colored" id="green" textColor="@color/button"
								text="绿"/>
						<button style="Widget.AppCompat.Button.Borderless.Colored" id="yellow" textColor="@color/button"
								text="黄"/>
						<button style="Widget.AppCompat.Button.Borderless.Colored" id="red" textColor="@color/button"
								text="红"/>
					</linear>
					<button style="Widget.AppCompat.Button.Borderless.Colored" id="recover" textColor="@color/button"
							text="恢复原状"/>
				</vertical>
		...
		warning.green.on("click", () => {
			warningPop.dismiss();
			threads.start(function () {
				//发送并返回数据
				try {
					sendAndReceive(formId, "绿");
				} catch (e) {
					finishTime = new Date().getTime();
					events.broadcast.emit("httpRes", {
						state: false,
		...
		warning.yellow.on("click", () => {
			warningPop.dismiss();
			threads.start(function () {
				//发送并返回数据
				try {
					sendAndReceive(formId, "黄");
				} catch (e) {
					finishTime = new Date().getTime();
					events.broadcast.emit("httpRes", {
						state: false,
		...
		warning.red.on("click", () => {
			warningPop.dismiss();
			threads.start(function () {
				//发送并返回数据
				try {
					sendAndReceive(formId, "红");
				} catch (e) {
					finishTime = new Date().getTime();
					events.broadcast.emit("httpRes", {
						state: false,
		...
		warning.recover.on("click", () => {
			warningPop.dismiss();
			threads.start(function () {
				//发送并返回数据
				try {
					sendAndReceive(formId, "");
				} catch (e) {
					finishTime = new Date().getTime();
					events.broadcast.emit("httpRes", {
						state: false,
		...
		<vertical fitsSystemWindows="true">
			<appbar w="*">
				<linear w="*" gravity="bottom">
					<toolbar id="toolbar" title="健康打卡" subtitle="健康码调色"/>
					...
					<vertical>
						...
						<vertical gravity="center" padding="15 10 15 0">
							...
							<vertical visibility="gone" id="httpRes" background="@drawable/white_background"
								...
								<vertical id="health" w="*" padding="5">
									<text w="*" paddingLeft="10" text="健康码状态"/>
									<input inputType="none" hint="未知" id="healthTxt" w="*"/>
								</vertical>
							</vertical>
						</vertical>
					</vertical>
